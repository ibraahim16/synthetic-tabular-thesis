...
def process_data(name):
    ...
    if info.get('use_pre_split', False):
        dummy_test_size = info.get("dummy_test_size", 4000)
        dummy_test_size = min(dummy_test_size, len(data_df))

        test_df = data_df.sample(n=dummy_test_size, random_state=42).copy()
